...
def gameover(message):  #結束程式
    ...
    screen.blit(text, (screen.get_width()/2-160,screen.get_height()/2-50))
...
pygame.init()
score = 0  #得分
font = pygame.font.SysFont("TT1139M_.TTF", 20)  #下方訊息字體
font1 = pygame.font.SysFont("TT1139M_.TTF", 32)  #結束程式訊息字體
...
for row in range(0, 4):  #3列方塊
    for column in range(0, 15):  #每列15磚塊
        if row == 0 or row == 1:  #1,2列為綠色磚塊
            brick = Brick((0,255,255), column * 40 + 1, row * 15 + 1)
        if row == 2 or row == 3:  #3,4列為藍色磚塊
            brick = Brick((255,0,255), column * 40 + 1, row * 15 + 1)
        bricks.add(brick)  #加入磚塊角色群組
        allsprite.add(brick)  #加入全部角色群組
msgstr = "Anonymous mouse left key start play!"  #起始訊息
...
while running:
    ...
    if playing == True:  #遊戲進行中
        ...
        if fail:  #球出界
            gameover("Failure, re-contact again!")
        pad.update()  #更新滑板位置
        ...
        if len(hitbrick) > 0:  #球和磚塊發生碰撞
            ...
            if len(bricks) == 0:  #所有磚塊消失
                gameover("Kyoki, successful challenge!")
    ...
    screen.blit(msg, (screen.get_width()/2-170,screen.get_height()-50))  #繪製訊息
